...
document.addEventListener("DOMContentLoaded", function() { 
    ...
    var ShowHitsComp = 5;
    const CatrsLong = CATALOG__HITS__Base.length;
    const arrHits = CATALOG__HITS__Base.slice(0, CatrsLong )
    ...
    var long  = CATALOG__HITS__Base.length
...
function CreadeCartHits(baseHits){
  ...
  function CreadeCartHitsBEfor(baseHits){
    baseHits.forEach(({id, name, price, img, art, sale}) => {
        let HtmlCatalog = '';
        HtmlCatalog +=`
        ...
        ROOT_PRODUCTSHIT.insertAdjacentHTML("afterbegin",HtmlCatalog);
      });
  };
