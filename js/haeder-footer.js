...
document.getElementById('ButtonEntrance').addEventListener("click", function(){
    ...
    if(element.login== 'admin12345'){
      if (UserLogin == element.login && UserPassword == element.password){
        document.getElementById('ButtonEntrance').setAttribute('href', 'admin.html');
        return
        
    }else if (UserLogin == element.login && UserPassword !== element.password){
      return
    }else if (UserLogin !== element.login && UserPassword == element.password){
      return
    }else if (UserLogin !== element.login && UserPassword !== element.password){
      ...
      if (UserLogin == element.login && UserPassword == element.password){
        let UserAcc ={};
        UserAcc.login = UserLogin;
        UserAcc.password = UserPassword;
...
/* window.addEventListener("click", function (event) {
  ...
  if (!event.target.matches(".haeder-burger")) {
    let Down_menu = document.querySelector('.menu-down-nav')
    Down_menu.classList.remove('menu-down-nav-active')
  }
});
 */
/* мобилка пущырьки ------------------------------------------ */
...
/* let a  = document.body
a.addEventListener('click', ButtonMobGoriz)
function ButtonMobGoriz(event){
  const Btn = event.target.closest('.svg-burger')
  let plaz = document.querySelector('.wp-nav')
  ButtonMobGoriz(plaz)
    if(!Btn){
      let Sub_Cat = document.querySelector('.menu-down-category')
